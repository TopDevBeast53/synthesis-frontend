...
const WalletInfo: React.FC<WalletInfoProps> = ({ hasLowBnbBalance, onDismiss }) => {
  ...
  const { balance: auraBalance, fetchStatus: auraFetchStatus } = useTokenBalance(tokens.aura.address)
  ...
  return (
    <>
      ...
      <Flex alignItems="center" justifyContent="space-between" mb="24px">
        <Text color="textSubtle">{t('AURA Balance')}</Text>
        {auraFetchStatus !== FetchStatus.Fetched ? (
          <Skeleton height="22px" width="60px" />
        ) : (
          <Text>{getFullDisplayBalance(auraBalance, 18, 3)}</Text>
        )}
      </Flex>
