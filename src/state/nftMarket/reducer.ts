...
>('nft/filterNftsFromCollection', async ({ collectionAddress, nftFilters }) => {
    try {
        ...
        const nftTokens: NftToken[] = Object.values(attrFilters.data).map((apiToken) => {
            const apiTokenMarketData = marketData.find(
                (tokenMarketData) => tokenMarketData.tokenId === apiToken.tokenId,
            )
...
export const NftMarket = createSlice({
    ...
    reducers: {
        ...
        setOrdering: (
            state,
            action: PayloadAction<{ collection: string; field: string; direction: 'asc' | 'desc' }>,
        ) => {
