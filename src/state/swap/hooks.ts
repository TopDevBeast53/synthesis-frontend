...
import { ChainId, Currency, CurrencyAmount, ETHER, JSBI, Token, TokenAmount, Trade } from 'sdk'
...
export function useDerivedSwapInfo(): {
    ...
    const { bestTrade: bestTradeExactIn } = useTradeExactInNoThrow(
        isExactIn ? parsedAmount : undefined,
        outputCurrency ?? undefined,
    )
    const { bestTrade: bestTradeExactOut } = useTradeExactOutNoThrow(
        inputCurrency ?? undefined,
        !isExactIn ? parsedAmount : undefined,
    )
...
function useTradeExactInNoThrow(
...
function useTradeExactOutNoThrow(
    currencyIn?: Currency,
