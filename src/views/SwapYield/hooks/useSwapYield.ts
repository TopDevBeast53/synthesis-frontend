...
export const useYieldSwap = () => {
  ...
  const openSwap = useCallback(async (exToken, poolId, amount, ask, lockDuration) => {
    const tx = await callWithGasPrice(getYieldSwapContract(), 'openSwap', [exToken, poolId, amount, ask, lockDuration])
    return tx.wait()
  }, [getYieldSwapContract, callWithGasPrice])

  const setAsk = useCallback(async (swapId, ask) => {
    const tx = await callWithGasPrice(getYieldSwapContract(), 'setAsk', [swapId, ask])
    return tx.wait()
  }, [getYieldSwapContract, callWithGasPrice])

  const closeSwap = useCallback(async (swapId) => {
    const tx = await callWithGasPrice(getYieldSwapContract(), 'closeSwap', [swapId])
    return tx.wait()
  }, [getYieldSwapContract, callWithGasPrice])

  const makeBid = useCallback(async (swapId, amount) => {
    const tx = await callWithGasPrice(getYieldSwapContract(), 'makeBid', [swapId, amount])
    return tx.wait()
  }, [getYieldSwapContract, callWithGasPrice])

  const setBid = useCallback(async (bidId, amount) => {
  ...
  const getSwap = useCallback(async (swapId) => {
  ...
  const applySellerFee = useCallback(async (amount) => {
    const tx = await callWithGasPrice(getYieldSwapContract(), 'applySellerFee', [amount])
    return tx.wait()
  }, [getYieldSwapContract, callWithGasPrice])

  const applyBuyerFee = useCallback(async (amount) => {
