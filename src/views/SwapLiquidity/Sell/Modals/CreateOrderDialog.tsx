...
import { AutoRenewIcon, BalanceInput, Button, Modal, Skeleton, Text } from 'uikit'
...
const AddRowModal = (props) => {
  ...
  const handleUAmountChange = useCallback(
    (e: React.FormEvent<HTMLInputElement>) => {
      if (e.currentTarget.validity.valid) {
        if (e.currentTarget.value === '') setUAmount('0')
  ...
  const handleBuyOptionChange = (option) => {
    setSelectedLPBuyOption(option)
  }
  ...
  const handleConfirm = async () => {
    ...
    if (selectedLPAllowance.lte(decimalUAmount)) {
      ...
      selectedLPContract
        .approve(LpSwapContract.address, ethers.constants.MaxUint256)
        .then(async (tx) => {
          await tx.wait()
    ...
    setPendingTx(true)
    LpSwapContract.openSwap(
      selectedLPOption.contract.address,
      selectedLPBuyOption.contract.address,
  ...
  useEffect(() => {
    ...
    Promise.all(allowanceContracts).then((allowances) => {
      ...
      else if (!selectedLPBuyOption) setSelectedLPBuyOption(tempLPOptions[1])
    })
