...
const ExternalRouterSelectPanel: React.FC<ExternalRouterSelectPanelProps> = ({
  ...
  const [onPresentExternalRoutersModal] = useModal(
    <ExternalRouterSelectModal onExternalRouterSelect={onExternalRouterSelect} externalRouter={externalRouter} />,
  )
  ...
  return (
    <ExternalRouterSelectButton endIcon={<ChevronDownIcon />} onClick={onPresentExternalRoutersModal}>
      <Row>
        {externalRouter && (
          <RouterLogo routerAddress={externalRouter.routerAddress} size="24px" style={{ marginRight: '8px' }} />
        )}
        <Text id="pair" bold>
