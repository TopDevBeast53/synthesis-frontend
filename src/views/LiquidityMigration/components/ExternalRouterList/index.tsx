...
const ExternalDexRow: React.FC<ExternalRouterRowProps> = ({
  ...
  return (
    <MenuItem style={style} onClick={() => (isSelected ? null : handleSelect())} selected={isSelected}>
      <RouterLogo routerAddress={rowExternalRouter.pairToken.address} size="24px" style={{ marginRight: '8px' }} />
      <Column>
        <Text bold>{rowExternalRouter.name}</Text>
...
const ExternalRouterList: React.FC<ExternalRouterListProps> = ({
  ...
  const Row = useCallback(
    ...
    [externalRouter, onExternalRouterSelect],
  )
