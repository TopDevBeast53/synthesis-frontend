...
export const useMigrateLiquidity = () => {
    ...
    const handleMigrateLiquidity: (externalRouter: ExternalRouterData) => Promise<ethers.providers.TransactionReceipt> =
        useCallback(
            async (externalRouter) => {
                ...
                const lpContract = new Contract(
                    externalRouter.pairToken.address,
                    helixPair,
                    getProviderOrSigner(library, account),
                )

                const approveTx = await callWithGasPrice(lpContract, 'approve', [
                    helixMigratorAddress,
