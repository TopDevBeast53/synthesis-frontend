...
import { getGraphLotteries } from 'state/lottery/helpers'
...
const PreviousRoundCardFooter: React.FC<{ lotteryNodeData: LotteryRound; lotteryId: string }> = ({
  ...
  if (lotteryNodeData) {
    const { amountCollectedInHelix } = lotteryNodeData
    prizeInBusd = amountCollectedInHelix.times(cakePriceBusd)
  }
      ...
      <>
        ...
        {prizeInBusd.isNaN() ? (
          ...
          <Balance
            ...
            color="textSubtle"
            unit=" HELIX"
            value={getBalanceNumber(lotteryNodeData?.amountCollectedInHelix)}
            decimals={0}
          />
        )}
      </>
