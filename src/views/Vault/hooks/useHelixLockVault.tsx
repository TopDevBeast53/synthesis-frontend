...
export const useHelixLockVault = () => {
  ...
  const getDepositFromId = useCallback(async (depositId) => {
  ...
  const getPendingRewardFromId = useCallback(async (depositId) => {
    const tx = await callWithGasPrice(getHelixAutoPoolContract(), 'pendingReward', [depositId])
    return tx.toString()
  },[getHelixAutoPoolContract, callWithGasPrice])

  const claimReward = useCallback(async (depositId) => {    
  ...
  const withdraw = useCallback(async (amount, depositId) => {
