...
export const useNFTBridge = () => {
    ...
    const getUnstakedNftsFromBSC = useCallback(async () => {
        ...
        if (tokenIds.length > 0) {
            const resTokensInfo = await Promise.all(
                tokenIds.map((e: any) => callWithGasPrice(getHelixNFTContract(), 'getToken', [e.id])),
            )
            const externalTokenIdsInfo = await Promise.all(
                tokenIds.map((e: any) => callWithGasPrice(getHelixNFTContract(), 'getExternalTokenID', [e.id])),
            )
            const approvedAddrsInfo = await Promise.all(
                tokenIds.map((e: any) => callWithGasPrice(getHelixNFTContract(), 'getApproved', [e.id])),
            )
            const tokens = resTokensInfo
                .map((token: any, i) => ({
    ...
    const approveToBridgeContract = useCallback(
        async (tokenId) => {
            const tx = await callWithGasPrice(getHelixNFTContract(), 'approve', [
                getHelixNFTBridgeContract().address,
                tokenId,
            ])
            return tx.wait()
        },
    ...
    const bridgeToSolana = useCallback(
        async (tokenId: string, externalOwnerAddr: string) => {
            const tx = await callWithGasPrice(getHelixNFTBridgeContract(), 'bridgeToSolana', [
                tokenId,
                externalOwnerAddr,
            ])
            return tx.wait()
    ...
    const bridgeToBSC = useCallback(
        async (externalTokenId, uri) => {
            const tx = await callWithGasPrice(getHelixNFTBridgeContract(), 'bridgeToBSC', [
                externalTokenId,
                account,
                uri,
            ])
    ...
    const mintBridgedNFT = useCallback(
        async (externalTokenId) => {
