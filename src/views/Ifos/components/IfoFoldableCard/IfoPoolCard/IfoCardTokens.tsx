...
const CommitTokenSection: React.FC<TokenSectionProps & { commitToken: Token }> = ({ commitToken, ...props }) => {
  if (commitToken.equals(cakeBnbLpToken)) {
    return <TokenSection primaryToken={tokens.cake} secondaryToken={tokens.weth} {...props} />
  }
...
const IfoCardTokens: React.FC<IfoCardTokensProps> = ({
  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(
    t(
      'Sorry, you didn’t contribute enough HELIX to meet the minimum threshold. You didn’t buy anything in this sale, but you can still reclaim your HELIX.',
    ),
    { placement: 'bottom' },
  )
    ...
    if (ifo.version === 3 && getBalanceNumber(credit) === 0) {
      message = (
        <Message my="24px" p="8px" variant="danger">
          <Box>
            <MessageText display="inline">
              {t('You don’t have any average HELIXbalance available to commit in the IFO HELIXpool.')}
            </MessageText>{' '}
      ...
      return userPoolCharacteristics.amountTokenCommittedInLP.isEqualTo(0) ? (
        <Flex flexDirection="column" alignItems="center">
          ...
          <Text textAlign="center" fontSize="14px">
            {t('To participate in the next IFO, stake some HELIXin the IFO HELIXpool!')}
          </Text>
