...
export const useGetNftInfo = () => {
  ...
  const getHelixNftInfoById = useCallback(async (tokenId: string) => {
  ...
  const getTokensOfOwnerByIds = useCallback(async (tokenIds) => {
    const results: any = await Promise.all(tokenIds.map((e:any)=>callWithGasPrice(getHelixNFTContract(), 'getToken', [e.id])))
  ...
  const getTokens = useCallback(async () => {
    ...
    if (tokenIds.length>0){
      const results = await Promise.all(tokenIds.map((e:any)=>callWithGasPrice(getHelixNFTContract(), 'getToken', [e.id])))
