...
const farmsToTest: [number, SerializedFarm][] = getFarms
...
describe('Config farms', () => {
    it.each(farmsToTest)('Farm #%d has an unique pid', (pid) => {
        const duplicates = getFarms.filter((f) => pid === f.pid)
        expect(duplicates).toHaveLength(1)
    })

    it.each(farmsToTest)('Farm #%d has an unique address', (pid, farm) => {
        const duplicates = getFarms.filter((f) => farm.lpAddress[56] === f.lpAddresses[56])
        expect(duplicates).toHaveLength(1)
    })
    ...
    it.each(farmsToTest)('Farm %d has the correct token addresses', async (pid, farm) => {
        ...
        const lpContract = getLpContract(farm.lpAddress[56])
    ...
    const START_PID = 365
    const FACTORY_ADDRESS = '0x968Ff589F8cd372907f3e1B9efF9b059b39FbAf3'
